...
export default function TokenPalette({ onTokenDragStart }: TokenPaletteProps) {
  const [activeCategory, setActiveCategory] = useState<Category>("characters");
  const filteredTokens = Object.entries(tokenData)
    .filter(([, value]) => value.category === activeCategory);
          ...
          <div className="grid grid-cols-4 sm:grid-cols-4 gap-2">
            {filteredTokens.map(([tokenId, { icon, color }]) => (
              <div
                ...
                draggable
                onDragStart={(e) => {
                  e.dataTransfer.setData("token-id", tokenId);
                  e.stopPropagation();
                  onTokenDragStart?.();
                }}
